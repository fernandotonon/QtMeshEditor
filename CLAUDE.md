...
qtmesh uv model.fbx --info                     # report current UV channels + UV0 bounding-box coverage
...
qtmesh rig model.obj --algo rignet -o rigged.fbx  # ML skeleton prediction via ONNX RigNet (#408); default --algo pinocchio (offline). RigNet falls back to the template when the model/ONNX is unavailable
