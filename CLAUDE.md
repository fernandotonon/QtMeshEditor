...
qtmesh convert model.fbx -o model.gltf2 --no-optimize  # skip vertex-cache/overdraw/fetch passes (#399)
...
qtmesh fix model.fbx --no-optimize             # preserve source triangle order on export
...
- **ExportOptimizer** (`src/ExportOptimizer.h/cpp`, issue #399): On-export pipeline that runs `meshopt_optimizeVertexCache` → `meshopt_optimizeOverdraw` (threshold 1.05) → `meshopt_optimizeVertexFetchRemap` on every submesh of an entity before the format-specific serializer reads the buffers. Hooked into `MeshImporterExporter::exporter(node, uri, format, stripAnimations, optimizeOnExport=true)`. Disable via `optimizeOnExport=false`, CLI `--no-optimize` (on `convert`, `fix`, `lod`), or MCP `optimize: false` on export tools. Vertex-fetch is only applied when the submesh owns its vertex data (not `useSharedVertices`) since remapping shared verts would scramble other submeshes' indices. `qtmesh info --json` now includes `submeshAcmr[]` per submesh so downstream tooling can decide whether to re-export. Sentry breadcrumb category `ai.assist.optimize_export`. Note: FBX writes go through Assimp which re-orders triangles on import, so the on-disk file's ACMR may not reflect the optimization — glTF/GLB and Ogre `.mesh` preserve the order.
