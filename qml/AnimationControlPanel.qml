...
Column {
    ...
    // doors, spinning machinery, camera moves, animated lights. Independent of
    // the selected mesh: node clips are owned by the SceneManager. The chosen
    // clip drives the dope-sheet "Node Transforms" band via
    // NodeAnimationManager.activeClip; keys land through the undoable command
    // path so Ctrl+Z works. Workflow: New clip → select a scene node → move it
    // with the gizmo → scrub the timeline → Key → Play.
    Rectangle { width: parent.width; height: 1; color: AnimationControlController.borderColor; opacity: 0.4 }
    ...
    Column {
        ...
        // Inspector-themed text input — mirrors the KFTransformField input
        // look (AnimationControlController palette) so the New-clip fields
        // match the rest of the panel instead of the default Controls style.
        ...
        function commitNewClip() {
            ...
            if (nm.length > 0 && !isNaN(len) && len > 0) {
                if (NodeAnimationManager.createClipUndoable(nm, len)) {
                    NodeAnimationManager.activeClip = nm
        ...
        Connections {
            ...
            function onClipsChanged() {
                ...
                // If the active clip vanished (deleted/undo), clear selection.
        ...
        Text {
            text: "Node Transform Animation"
            color: AnimationControlController.textColor
            font.pixelSize: 11; font.bold: true
        }

        // Clip picker (themed dropdown) + New / Delete (ToolBtn).
        Row {
            spacing: 4; width: parent.width

            // Themed clip dropdown — same Rectangle+Popup idiom as the
            // Animation typeahead above.
            Item {
                id: nodeClipSelector
        ...
        // Inline "new clip" editor row (themed) — replaces a default-styled
        // modal Dialog so it matches the Inspector look.
        Row {
        ...
        // Play toggle (themed) + key the selected node.
        Row {
            spacing: 6; width: parent.width
            visible: NodeAnimationManager.activeClip.length > 0

            // Themed Play checkbox — flat 16px box + checkmark, palette colors.
            Row {
                spacing: 5
                anchors.verticalCenter: parent.verticalCenter
                Rectangle {
                    ...
                    anchors.verticalCenter: parent.verticalCenter
                    property bool on: NodeAnimationManager.isClipEnabled(NodeAnimationManager.activeClip)
                    color: on ? AnimationControlController.highlightColor
                              : AnimationControlController.inputColor
                    border.color: AnimationControlController.borderColor; border.width: 1
                    // Re-read the enabled state whenever the active clip changes.
                    Connections {
                        target: NodeAnimationManager
                        function onActiveClipChanged() {
                            nodePlayBox.on = NodeAnimationManager.isClipEnabled(NodeAnimationManager.activeClip)
                        }
                    }
                    Text {
                        anchors.centerIn: parent; text: "✓"
                        visible: nodePlayBox.on
                        color: AnimationControlController.buttonTextColor; font.pixelSize: 11; font.bold: true
                    }
                    MouseArea {
                        anchors.fill: parent
                        onClicked: {
                            nodePlayBox.on = !nodePlayBox.on
                            NodeAnimationManager.setClipEnabled(
                                NodeAnimationManager.activeClip, nodePlayBox.on)
                        }
                    }
                }
                Text {
                    text: "Play"; anchors.verticalCenter: parent.verticalCenter
                    color: AnimationControlController.textColor; font.pixelSize: 11
                }
            }
        ...
        Text {
            ...
            text: PropertiesPanelController.selectionName.length > 0
                  ? "Target: " + PropertiesPanelController.selectionName +
                    " — move it, set the playhead, then Key. Untick Play to edit."
                  : "Select a scene node to key its transform."
            color: AnimationControlController.disabledTextColor
            font.pixelSize: 10
        }
    }
