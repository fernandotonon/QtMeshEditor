...
if grep -RInE '(^|[^a-zA-Z_])(import|from)[[:space:]]+nvdiff(rast|rec)' \
        ...
        ai/trellis2 src/ImageTo3D 2>/dev/null; then
...
if grep -RInE '^[[:space:]]*(nvdiffrast|nvdiffrec|cumesh)([=<>![:space:];[]|$)' \
        ai/trellis2/requirements.txt 2>/dev/null; then
...
if grep -InE 'nvdiff(rast|rec)' \
        ...
        src/ImageTo3D/Trellis2Interchange.h 2>/dev/null \
...
if grep -InE 'nvdiffrast\.git|nvdiffrec\.git|pip.*nvdiff' ai/trellis2/install.py 2>/dev/null; then
