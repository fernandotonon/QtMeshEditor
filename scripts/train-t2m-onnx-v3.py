...
def main():
    ...
    class T2M(nn.Module):
        def __init__(s, dim=256, layers=4, heads=8):
            ...
            # DELTA HEAD (v3): instead of predicting each frame's absolute pose
            # independently (v1/v2 — frames could jump/wobble since nothing tied
            # frame t to t-1), predict the frame-0 ABSOLUTE pose + a per-frame
            # DELTA, then integrate by cumulative sum. Output is continuous BY
            # CONSTRUCTION (frame t = frame0 + Σ deltas), so the trajectory can't
            # discontinuously jump — the structural fix for the late-frame wobble
            # that loss penalties alone (v2) only softened. Still one forward pass
            # (cumsum is ONNX-friendly), so the C++ export contract is unchanged.
            s.out0 = nn.Linear(dim, D6)      # absolute frame-0 pose
            s.outd = nn.Linear(dim, D6)      # per-frame delta (delta[0] unused)
        def encode(s, m):
        ...
        def decode(s, tokens, z):
            ...
            for layer in s.dec:
                x = layer(x, cond)
            base = s.out0(x[:, :1, :])                               # [B,1,D6] frame-0 pose
            deltas = s.outd(x)                                       # [B,T,D6]
            deltas = torch.cat([torch.zeros_like(deltas[:, :1]), deltas[:, 1:]], 1)
            return base + torch.cumsum(deltas, dim=1)                # [B,T,D6] integrated
    ...
    def accel(x6):                          # 2nd difference magnitude (jerkiness)
        d = x6[:, 1:] - x6[:, :-1]
    ...
    for ep in range(a.epochs):
        ...
        for b in range(0, steps_per_epoch * bs, bs):
            ...
            # TEMPORAL COHERENCE (v2): the one-shot decoder drifted/collapsed in
            # LATE frames (good early, garbage later). Two added terms:
            #  - amatch: match the target's ACCELERATION (2nd diff) so the
            #    trajectory is as smooth as real motion, not erratic;
            #  - late: weight the recon error of the SECOND HALF of the clip up,
            #    so late-frame collapse is punished instead of averaged away by
            #    good early frames.
            amatch = (accel(pred) - accel(tgt)).abs()
            half = pred.shape[1] // 2
            late = ((pred[:, half:] - tgt[:, half:]) ** 2).mean()
            # v3 DELTA-MAGNITUDE CONTROL: the cumsum-integrated decoder tends to
            # OVERSHOOT (genvel ~4x real) because per-frame delta errors accumulate.
            # Strongly match the generated velocity to the target's AND penalise
            # any excess (one-sided) so the integrated trajectory has real motion
            # magnitude — keeps the high action-distinctness the delta form gives
            # while killing the jitter.
            vexcess = F.relu(vel(pred) - vel(tgt))
            loss = recon + beta * kl + 2.0 * vmatch + 0.5 * vfloor \
                   + 0.5 * amatch + 0.5 * late + 2.0 * vexcess
