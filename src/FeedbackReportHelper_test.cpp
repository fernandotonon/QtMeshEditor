...
TEST_F(FeedbackReportHelperTest, ResetForTestsClearsHandlerWithoutInvocation)
{
    int callCount = 0;
    FeedbackReportHelper::setOpenFeedbackHandler(
        [&callCount](const FeedbackPrefill&) { ++callCount; });

    // resetForTests must clear the stored handler. Since there is no public
    // way to invoke the handler without a display, we observe the state
    // change indirectly: setting then resetting must not invoke the handler,
    // and a subsequent re-set/reset cycle must also be safe.
    FeedbackReportHelper::resetForTests();

    EXPECT_EQ(callCount, 0);
}

TEST_F(FeedbackReportHelperTest, SetHandlerThenResetIsRepeatableAndSafe)
{
    int callCount = 0;
    auto handler = [&callCount](const FeedbackPrefill&) { ++callCount; };

    for (int i = 0; i < 3; ++i) {
        FeedbackReportHelper::setOpenFeedbackHandler(handler);
        FeedbackReportHelper::resetForTests();
    }

    // No invocation path was triggered (showFailureWithReportOption not called).
    EXPECT_EQ(callCount, 0);
}

TEST_F(FeedbackReportHelperTest, SetNullHandlerIsAccepted)
{
    // Assigning an empty std::function must be accepted and not crash.
    FeedbackReportHelper::setOpenFeedbackHandler(
        FeedbackReportHelper::OpenFeedbackHandler{});
    FeedbackReportHelper::resetForTests();
    SUCCEED();
}
